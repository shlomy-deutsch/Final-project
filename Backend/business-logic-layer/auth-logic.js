...
async function addProductAsync(product) {
  // console.log({product});
  const sql = `INSERT INTO castumers (castumer_ID, first_Name, last_Name, email, password, city, street, admin, open)
...
async function loginAsync(credentials) {
  const sql = `SELECT castumer_ID, first_Name, street, city, email, admin, open FROM castumers WHERE email = '${credentials.username}' AND password = '${credentials.password}' LIMIT 1`;
